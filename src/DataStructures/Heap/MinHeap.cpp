...
const sf::Vector2f MinHeap::TREE_OFF_SET(35.f, 100.f);
const unsigned int MinHeap::MAX_SIZE(31);
const int MinHeap::MAX_VALUE(999);
const int MinHeap::MIN_VALUE(1);

MinHeap::Height::Height(int left, int right) : left(left), right(right) {}

void MinHeap::Height::updateLeft(const MinHeap::Height& child) {
	left = std::max(child.left, child.right) + 1;
}

void MinHeap::Height::updateRight(const MinHeap::Height& child) {
	right = std::max(child.left, child.right) + 1;
}

MinHeap::MinHeap(const FontHolder& fonts, const ColorHolder& colors)
    : mNodes(), mFonts(fonts), mColors(colors), height(), mBin(nullptr) {
	randomize();
}

void MinHeap::randomize() {
	std::vector<int> elements(Random::getArray(1, MinHeap::MAX_SIZE, MIN_VALUE, MAX_VALUE));
	loadArray(elements);
}

void MinHeap::loadFromFile(const std::string& fileDir) {
...
std::pair<std::vector<Animation>, std::string> MinHeap::pushAnimation(const int& value) {
	const std::string& code = HeapCode::Min::Push;
...
std::pair<std::vector<Animation>, std::string> MinHeap::deleteAnimation(const int& id) {
	...
	const std::string& code = HeapCode::Min::Delete;
	std::vector<Animation> list;
	list.push_back(Animation(
	...
	/* down heap */
	{
		...
		while (index < values.size()) {
			...
			if (leftChild < values.size() && values[smallest] > values[leftChild])
				smallest = leftChild;
			if (rightChild < values.size() && values[smallest] > values[rightChild])
				smallest = rightChild;
			if (smallest != index) {
				list.push_back(Animation(
				    {5},
				    "Smallest child of A[" + std::to_string(index) + "] is A[" +
				        std::to_string(smallest) + "]",
				    [&, index, smallest, last]() {
					    if (last != -1)
						    mNodes[last]->highlight(PolyNode::None);
					    mNodes[index]->highlight(PolyNode::Primary);
					    mNodes[smallest]->highlight(PolyNode::Secondary);
				    },
				    [&, index, smallest, last]() {
					    if (last != -1)
						    mNodes[last]->highlight(PolyNode::Secondary);
					    mNodes[index]->highlight(PolyNode::Primary);
					    mNodes[smallest]->highlight(PolyNode::None);
				    }));
				list.push_back(Animation(
				    {6, 7},
				    "Swap A[" + std::to_string(index) + "] with A[" + std::to_string(smallest) +
				        "], then move to A[" + std::to_string(smallest) + "]",
				    [&, index, smallest, last]() {
					    mNodes[index]->swapData(mNodes[smallest]);
					    mNodes[index]->highlight(PolyNode::Secondary);
					    mNodes[smallest]->highlight(PolyNode::Primary);
				    },
				    [&, index, smallest]() {
					    mNodes[index]->swapData(mNodes[smallest]);
					    mNodes[index]->highlight(PolyNode::Primary);
				...
				std::swap(values[smallest], values[index]);
				last = index;
				index = smallest;
			} else {
				list.push_back(Animation(
...
std::pair<std::vector<Animation>, std::string> MinHeap::getTopAnimation() {
...
std::pair<std::vector<Animation>, std::string> MinHeap::getSizeAnimation() {
...
void MinHeap::loadArray(const std::vector<int>& array) {
	clear();
	for (int v : array)
		push(v);
}

void MinHeap::clear(const int& root) {
	if (mNodes.empty())
		return;
...
void MinHeap::clearHighlight() {
	for (auto& node : mNodes)
		node->highlight(PolyNode::None);
}

void MinHeap::alignBinaryTree() {
...
void MinHeap::push(const int& value) {
	if (mNodes.size() == MAX_SIZE) {
		std::cerr << "MinHeap maximum size reached!\n";
		return;
	}
...
void MinHeap::purePush(const int& value) {
	//	std::cout << "MinHeap push: " << value << '\n';
...
void MinHeap::remove(const int& index) {
...
void MinHeap::heapifyDown() {
	...
	while (index < mNodes.size()) {
		...
		if (leftChild < mNodes.size() &&
		    mNodes[smallest]->getIntData() > mNodes[leftChild]->getIntData())
			smallest = leftChild;
		if (rightChild < mNodes.size() &&
		    mNodes[smallest]->getIntData() > mNodes[rightChild]->getIntData())
			smallest = rightChild;
		if (smallest != index) {
			std::string temp = mNodes[index]->getData();
			mNodes[index]->setData(mNodes[smallest]->getData());
			mNodes[smallest]->setData(temp);
			index = smallest;
		} else
			break;
	}
...
int MinHeap::parent(const int& index) {
	return (index - 1) / 2;
}

void MinHeap::dump(PolyNode* node) {
	node->removeAllEdges();
	node->setTargetScale(0.f, 0.f, Transition::Smooth);
	mBin = node;
}

void MinHeap::updateCurrent(sf::Time dt) {
